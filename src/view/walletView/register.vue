<template>
  <div id="login" class="login_form padding20 full-screen">
    ...
    <div class="title flex align between">
      <div class="h2 text_color_dark">{{$t('wallet.register.title')}}</div>
    ...
    <div class="them_form">
      ...
      <div class="input_gorup">
        <input type="text" :placeholder="$t('wallet.register.form_input_userName')" v-model="form.userAccount"  @blur="blur_event">
      </div>

      <div class="input_gorup">
        <input type="text"  :placeholder="$t('wallet.register.form_input_email')" v-model="form.phone"  @blur="blur_event">
      ...
      <div class="input_gorup">
        <input type="number" style="-webkit-text-security:disc" :placeholder="$t('feature.register.input_payPass')" v-model="form.transactionPwd"  @blur="blur_event">
      </div>

      <div class="input_gorup">
        <input type="number" style="-webkit-text-security:disc" :placeholder="$t('feature.register.input_payPass_re')" v-model="transactionPwd_re"  @blur="blur_event">
      </div>

      <div class="input_gorup">
        <input type="text" :placeholder="$t('wallet.register.form_input_invitation')" v-model="form.invitation" :disabled="invitation_dis"  @blur="blur_event">
         <div class="color-warning font12 marginT10 animated fadeIn" v-if="recommendreName">{{$t('feature.register.text_recommendreName')}}：{{recommendreName}}</div>
      </div>
      ...
      <div class="space20"></div>
      <van-button type="info" @click="submitRegister()">{{$t('wallet.register.form_btn')}}</van-button>
      <div class="space30"></div>

      <div class="info flex align between">
        <div class="textLeft">
          <span class="font14 them_color_gray">{{$t('wallet.register.old_user')}}</span>&nbsp;
          <a @click="gopage_re('login')" class="font14 font_link">{{$t('wallet.register.login_now')}}</a>
        </div>
        
        <a  class="font14 font_link" href="https://www.gscapp.cn/ios/cAxn" target="_bank">{{$t('feature.register.text_download')}}</a>
