<template>
  <div  v-if="currCoin">
    ...
    <div class="token_choose">
      <div class="token_name flex between align" @click="showList = true">
        <b class=" font16 font-bold">{{currCoin.coin.coinName}}</b>
        <small class="them_color_gray">{{$t('wallet.chooseCoins.text_title')}}
    ...
    <van-popup v-model="showList" position="right">
      <div class="coinsList">
        <div class="item border-bottom" :class="item.coin.coinName==currCoin.coin.coinName?'active':''" v-for="(item,index) in coinsArr" :key="index" @click="chooseCoin(item)">{{item.coin.coinName}}</div>
      </div>
    </van-popup>
    <!-- showList -->
